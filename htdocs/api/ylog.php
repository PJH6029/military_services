...
function doGET() {
    ...
    $getCalTxtQry = "SELECT * FROM ylogtexttable";
...
function doDEL() {
    ...
    $qry = sprintf("DELETE FROM ylogtexttable WHERE ymd = '%s'", $_DEL["ymd"]);
...
function doPUT() {
    ...
    $qry = sprintf("INSERT INTO ylogtexttable (ymd, content) VALUES ('%s', '%s') ON DUPLICATE KEY UPDATE content = '%s'", $_PUT["ymd"], $_PUT["content"], $_PUT["content"]);
