...
function doGET() {
    ...
    if (isset($_GET["cardId"])) {
        // retrieve one card
        $qry = sprintf("SELECT * FROM daily_card_table WHERE id = '%s'", $_GET["cardId"]);
    } else if (isset($_GET["keyword"])) {
        // search
        $serachKey = $_GET["keyword"];
        $qry = "SELECT * FROM daily_card_table WHERE content LIKE '%".$serachKey."%' OR TITLE LIKE '%".$serachKey."%'";
    } else if (isset($_GET["pageLength"])) {
        // pagination
        if (isset($_GET["lastCardStamp"])) {
            $qry = sprintf("SELECT * FROM daily_card_table WHERE STRCMP(created_at, '%s') = -1 ORDER BY created_at DESC LIMIT %d", $_GET["lastCardStamp"], $_GET["pageLength"]);
        } else {
            $qry = sprintf("SELECT * FROM daily_card_table ORDER BY created_at DESC LIMIT %d", $_GET["pageLength"]);
        }
    } else {
        $qry = "SELECT * FROM daily_card_table ORDER BY created_at DESC";
    }
...
function doPOST() {
    ...
    $qry = sprintf("INSERT INTO daily_card_table (title, content, writer_id) VALUES ('%s', '%s', '%s');", addslashes($_POST["title"]), addslashes($_POST["content"]), $_POST["writer_id"]);
    $qryResult = $conn->query($qry);
...
function doDEL() {
    ...
    $qry = sprintf("DELETE FROM daily_card_table WHERE id = %d", $_DEL["id"]);
    $qryResult = $conn->query($qry);
...
function doPUT() {
    ...
    $qry = sprintf("UPDATE daily_card_table SET title = '%s', content = '%s' WHERE id = %d", addslashes($_PUT["title"]), addslashes($_PUT["content"]), $_PUT["id"]);
    $qryResult = $conn->query($qry);
