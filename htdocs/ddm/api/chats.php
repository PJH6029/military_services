...
function doGET() {
    ...
    if (isset($_GET["chatUpdate"]) && !isset($_GET["limit"])) {
        $qry = sprintf("SELECT COUNT(id) FROM chat_table WHERE receiver_user_id = '%s';", $utfUser);
        $qryResult = $conn->query($qry);
        $row = $qryResult->fetch_row();
        $responseData->count = intval($row[0]);
    } else {
        if (isset($_GET["limit"])) {
            $qry = sprintf("SELECT * FROM chat_table WHERE receiver_user_id = '%s' ORDER BY id DESC LIMIT %d;", $utfUser, $_GET["limit"]);
        } else {
