...
function doGET() {
    ...
    $getCalTxtQry = "SELECT * FROM meeting_calender_table";
    ...
    while ($calRow = $getCalTxtQryResult->fetch_array(MYSQLI_ASSOC)) {
        ...
        $responseData->contents->$calYMD = $calContentUTF;
    }
...
function doDEL() {
    ...
    $qry = sprintf("DELETE FROM meeting_calender_table WHERE ymd = '%s'", $_DEL["ymd"]);
...
function doPUT() {
    ...
    $qry = sprintf("INSERT INTO meeting_calender_table (ymd, content) VALUES ('%s', '%s') ON DUPLICATE KEY UPDATE content = '%s'", $_PUT["ymd"], $_PUT["content"], $_PUT["content"]);
